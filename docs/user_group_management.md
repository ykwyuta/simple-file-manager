...
| `password` | String | ハッシュ化されたパスワード | BCryptでハッシュ化 |

**注意**: 管理者権限は、ユーザーが`admins`グループに所属しているかどうかで判定されます。専用の`is_admin`フィールドは存在しません。
...
-   **ユーザー削除時のファイル所有権**: ✅ **実装済み**
    -   ユーザーを削除した際に、そのユーザーが所有していたファイル/フォルダの所有者は自動的に`admin`ユーザーに移転されます。
    -   `admin`ユーザー自体は削除できないように保護されています。
    -   詳細は[ユーザー削除時の所有権移転](./user-deletion-ownership-transfer.md)を参照してください。
-   **グループ削除時のファイル所有グループ**: ✅ **実装済み**
    -   グループを削除した際に、そのグループが所有していたファイル/フォルダの所有グループは自動的に`admins`グループに移転されます。
    -   `admins`グループ自体は削除できないように保護されています。
-   **パスワード管理**: パスワードは必ずハッシュ化（BCrypt）してDBに保存します。平文での保存は行いません。
-   **デフォルトグループ**: 新規ユーザー作成時に、自動的に特定のデフォルトグループ（例: `users` グループ）に所属させる仕様も考えられます。
