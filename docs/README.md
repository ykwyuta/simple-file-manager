...
- **[security.md](./security.md)** - セキュリティ設計
  - 認証方式（画面はフォーム認証、APIはステートレスBasic認証）
  - 認可モデルとパーミッション判定の適用順
  - 入力検証の規則
  - 本番投入前チェックリスト

- **[operations.md](./operations.md)** - 運用ガイド
  - 起動方法（依存サービスなしのローカル起動を含む）
  - ストレージ実装の切り替え
  - 削除バッチの挙動と監視すべき点
  - バックアップとリストア、不整合時の対処

- **[migrations.md](./migrations.md)** - データベースマイグレーション
  - Flywayによるスキーマ管理と `ddl-auto=validate`
  - マイグレーションの追加手順と稼働中データを壊さない書き方
  - PostgreSQL / H2 両対応の可搬なSQL
  - Flyway導入前のデータベースの移行

- **[api.md](./api.md)** - APIリファレンス
  - エンドポイント一覧と必要な権限
  - エラー応答（RFC 7807）とステータスコードの規約
  - 制限値
