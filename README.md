...
2.  **フェーズ2: コア機能の実装**
    ...
    2.  **ファイル/フォルダ操作 (No. 2.1)**: ✅ **一部実装済み**
        -   S3連携を実装し、ファイルの**アップロード**と**ダウンロード**、**論理削除**機能（`POST /api/files`, `GET /api/files/{id}`, `DELETE /api/files/{id}`）を実装しました。
        -   今後、リネーム、移動の機能の実装が必要です。

3.  **フェーズ3: 付加価値機能の実装**
    1.  **ファイル検索 (No. 2.3)**: ✅ **実装済み**
        -   ファイル名とカスタムタグによる部分一致検索API (`GET /api/files/search`) を実装しました。
    2.  **削除処理の高度化 (No. 6)**:
        -   論理削除されたアイテムを復元する「ゴミ箱」機能を実装します。
        -   古い論理削除済みデータを物理削除するスケジュールバッチ処理を実装します。
