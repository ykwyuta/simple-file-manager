...
| **実行** | `x` (1) | ファイルを実行する (このシステムでは未使用)、フォルダにアクセスする/移動する |
  ...
  * **権限フィールド**: DBのファイル/フォルダのメタデータに、**所有者権限**、**グループ権限**、**その他権限**の3つのフィールド（例: 10進数`755`など）を持たせる。
...
| 5.5 | `permissions` | Integer | Linuxパーミッション (例: `755`) | 必須、3桁の10進数表記 |
| 5.6 | `storage_key` | String | S3互換ストレージ内のキー | ファイルの場合必須 |
| 5.7 | `custom_tags` | Text | ユーザーが任意に設定できる検索用タグ | 任意。カンマ区切り。 |
...
| 5.12| `versioning_enabled` | Boolean | バージョン管理が有効か | フォルダの場合のみ |
| 5.13| `is_locked` | Boolean | ファイルがロックされているか | デフォルト false |
| 5.14| `locked_by_user_id` | Long | ロックしたユーザーID | ロック時のみ設定 |
| 5.15| `locked_at` | Timestamp | ロック日時 | ロック時のみ設定 |
...
- [ ] **未実装機能のUI実装 (残タスク)**
    ...
    - [x] **ファイルロック機能**
        - [x] ファイル一覧でのロック状態表示 (アイコン等)
        - [x] ロック/アンロックを実行するアクション
        - [x] `WebController`へのロック処理エンドポイント追加
- [ ] **管理機能UI実装 (残タスク)**
    - [x] **ユーザー管理画面**
        - [x] ユーザー一覧、作成、編集、削除
    - [x] **グループ管理画面**
        - [x] グループ一覧、作成、編集、削除
- [ ] **UI/UX改善 (残タスク)**
    - [x] **パンくずリストの完全階層表示**: 現在は「Home / カレントフォルダ」のみ。親フォルダを遡ってリンク表示できるようにする。
    - [x] **エラーハンドリング**: エラーメッセージの表示改善、入力バリデーションのUI反映。
        - [x] アラートの自動消去と手動閉じるボタン
        - [x] クライアントサイドのフォームバリデーション（ファイル名、パーミッション）
        - [x] サーバーサイドの詳細な例外処理とユーザーフレンドリーなエラーメッセージ
    - [x] **ページネーション**: ファイル数が多い場合のページ送り。
        - [x] FileRepositoryにページネーション対応メソッドを追加
        - [x] FileServiceにページネーション対応メソッドを追加
        - [x] WebControllerでページネーションパラメータ(page, size)を受け取るように更新
        - [x] home.htmlにページネーションコントロール(前へ/次へボタン、ページ番号)を追加

### 12. 📋 作業一覧 （改善機能一覧）

- [x] ユーザが削除されて、所有ユーザがいなくなったファイル、フォルダはadminユーザの所有に変更する。
- [x] グループが削除されて、所有グループがいなくなったファイル、フォルダはadminユーザの所有に変更する。
- [x] adminsグループのユーザはファイル、フォルダの所有ユーザと所有グループを任意のユーザ、グループに変更できる。
- [x] フォルダの所有ユーザと所有グループを変更するときはその配下の所有ユーザ、所有グループも連動して変更するオプションを設ける。
- [x] ファイルにタグをつける機能を追加する。
