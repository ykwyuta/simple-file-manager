...
1.  **ファイルのバージョン管理機能**
    ...
    *   **実装タスクリスト**:
        1.  **データベーススキーマの拡張**:
            *   `FileEntity` (フォルダ) に、バージョン管理が有効かを示す `versioning_enabled` (Boolean) カラムを追加する。
            *   ファイルのバージョン履歴を格納する新しいエンティティ `FileHistory` を作成する。
                *   `id`: 主キー
                *   `file_entity_id`: 関連するファイルへの外部キー
                *   `version`: バージョン番号 (自動インクリメント)
                *   `storage_key`: S3上のファイル本体を指すキー
                *   `modifier_user_id`: 更新者のユーザーID
                *   `created_at`: バージョン作成日時
        2.  **APIエンドポイントの追加**:
            *   `PUT /api/folders/{id}/versioning`: フォルダのバージョン管理を有効化/無効化する。
            *   `GET /api/files/{id}/versions`: 特定のファイルのバージョン履歴を一覧で取得する。
            *   `POST /api/files/{id}/restore/{version_id}`: ファイルを特定のバージョンに戻す (実態は、指定バージョンを最新版として新規作成する)。
        3.  **サービスロジックの修正・追加**:
            *   ファイル更新処理の修正: ファイルを更新する際、親フォルダの `versioning_enabled` フラグをチェックする。
                *   `true` の場合: S3に新しいファイルを作成し、`FileHistory` にレコードを追加する。`FileEntity` の `storage_key` は最新バージョンのものを指すように更新する。
                *   `false` の場合: 従来通り、S3上のファイルを上書きする。
            *   バージョン復元ロジックの実装: 指定された `FileHistory` のレコードを元に新しいバージョンを作成し、最新のバージョンとして登録する。
